...
public final class VoiceMessagesFabricClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        if (VoiceMessages.isActive()) {
            ClientConfigurationNetworking.registerGlobalReceiver(VoiceMessagesConfigS2C.ID, (payload, context) ->
                    VoiceMessagesClientNetworking.onVoiceMessagesConfigReceived(payload));
            ClientPlayNetworking.registerGlobalReceiver(VoiceMessagesPermissionsS2C.ID, (payload, context) ->
                    VoiceMessagesClientNetworking.onVoiceMessagesPermissionsReceived(payload));
            ClientPlayNetworking.registerGlobalReceiver(VoiceMessageS2C.ID, (payload, context) ->
